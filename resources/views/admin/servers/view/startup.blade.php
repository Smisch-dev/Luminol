...
@section('footer-scripts')
    ...
    $(document).ready(function () {
        $('#pEggId').select2({placeholder: 'Select a Nest Egg'}).on('change', function () {
            ...
            var parentChain = _.get(Luminol.nests, $("#pNestId").val());
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (objectChain.id === parseInt(Luminol.server.egg_id) && Luminol.server.image == opt.value) {
                    opt.selected = true
                }
            ...
            if (objectChain.id === parseInt(Luminol.server.egg_id)) {
                if ($('#pDockerImage').val() != Luminol.server.image) {
                    $('#pDockerImageCustom').val(Luminol.server.image);
                }
            }
            ...
            $.each(_.get(objectChain, 'variables', []), function (i, item) {
                var setValue = _.get(Luminol.server_variables, item.env_variable, item.default_value);
        ...
        $('#pNestId').select2({placeholder: 'Select a Nest'}).on('change', function () {
            $('#pEggId').html('').select2({
                data: $.map(_.get(Luminol.nests, $(this).val() + '.eggs', []), function (item) {
                    return {
                        id: item.id,
                        text: item.name,
                    };
            ...
            if (_.isObject(_.get(Luminol.nests, $(this).val() + '.eggs.' + Luminol.server.egg_id))) {
                $('#pEggId').val(Luminol.server.egg_id);
            }
