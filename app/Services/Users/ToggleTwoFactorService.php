...
use Luminol\Repositories\Eloquent\RecoveryTokenRepository;
use Luminol\Exceptions\Service\User\TwoFactorAuthenticationTokenInvalid;
...
class ToggleTwoFactorService
{
     ...
     * @throws \Luminol\Exceptions\Service\User\TwoFactorAuthenticationTokenInvalid
     */
    public function handle(User $user, string $token, bool $toggleState = null): array
    {
        $secret = $this->encrypter->decrypt($user->totp_secret);

        $isValidToken = $this->google2FA->verifyKey($secret, $token, config()->get('luminol.auth.2fa.window'));
